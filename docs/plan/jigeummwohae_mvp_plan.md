...
공개 채팅방은 **그룹채팅과 1:1 채팅 모집방**을 지원한다. 1:1 모집방은 여러 사용자가 발견할 수 있지만 각 참여자는 방장과 독립된 1:1 대화방에서 대화한다.
...
지도에는 하나의 방으로 노출되지만 참여자마다 방장과 별도의 1:1 대화방이 생성된다. 여러 참여자는 서로의 대화나 존재를 볼 수 없다.
